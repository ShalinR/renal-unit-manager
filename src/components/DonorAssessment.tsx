...
const DonorAssessment: React.FC<DonorAssessmentProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setActiveView("dashboard"); // Navigate back to the dashboard
  };
  // Helper for nested field updates (e.g., comorbidities.dm)
  const handleNestedChange = (fieldPath: string, value: any) => {
  ...
  {FORM_STEPS.map((label, idx) => (
    <div
      ...
      className={`flex-1 text-center px-4 py-2 rounded-lg text-sm font-medium transition-colors
        ${step === idx ? "bg-blue-600 text-white shadow" : "bg-gray-100 text-gray-700"}`}
    >
      {label}
    </div>
  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      // optional: validate dd/mm/yyyy format here
                      handleDonorFormChange('dateOfBirth', e.target.value);
                    }}
                    required
                  />
                </div>
