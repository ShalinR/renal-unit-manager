...
const HDScheduleAppointment: React.FC<HDScheduleAppointmentProps> = ({
  onBack,
}) => {
  ...
  const slotTaken = (slotId: string) =>
    todaysAppointments.some((a) => a.slotId === slotId);
  ...
  const handleBook = () => {
    ...
    if (!selectedSlot) {
      toast({
        title: "Select Slot",
        description: "Please pick a time slot to schedule.",
        variant: "destructive",
      });
      return;
    }
    ...
    (async () => {
      try {
        ...
        toast({
          title: "Booked",
          description: `Appointment booked for ${patient.name} on ${date} (${selectedSlot})`,
        });
        ...
        <CardContent className="p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="mt-1">
                <Input
                  value={patient?.name || ""}
                  placeholder="Select patient from search"
                  disabled
                />
              </div>
            </div>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Any notes for the session"
            />
          </div>
