...
export const DonorDetailsModal: React.FC<DonorDetailsModalProps> = ({
  ...
  const getComorbiditiesList = () => {
    ...
    if (donorData.comorbidities?.ihd)
      comorbidities.push("Ischemic Heart Disease");
    if (donorData.comorbidities?.psychiatricIllness)
      comorbidities.push("Psychiatric Illness");
  ...
  const getSystemicInquiryList = () => {
    ...
    // Constitutional
    if (donorData.systemicInquiry?.constitutional?.loa)
      symptoms.push("Loss of Appetite");
    if (donorData.systemicInquiry?.constitutional?.low)
      symptoms.push("Loss of Weight");
    ...
    if (donorData.systemicInquiry?.cvs?.sob)
      symptoms.push("Shortness of Breath");

    // Respiratory
    if (donorData.systemicInquiry?.respiratory?.cough) symptoms.push("Cough");
    if (donorData.systemicInquiry?.respiratory?.hemoptysis)
      symptoms.push("Hemoptysis");
    if (donorData.systemicInquiry?.respiratory?.wheezing)
      symptoms.push("Wheezing");

    // GIT
    if (donorData.systemicInquiry?.git?.constipation)
    ...
    if (donorData.systemicInquiry?.git?.prBleeding)
      symptoms.push("Per Rectal Bleeding");

    // Renal
    if (donorData.systemicInquiry?.renal?.hematuria) symptoms.push("Hematuria");
    if (donorData.systemicInquiry?.renal?.frothyUrine)
      symptoms.push("Frothy Urine");

    // Neurological
    if (donorData.systemicInquiry?.neuro?.seizures) symptoms.push("Seizures");
    if (donorData.systemicInquiry?.neuro?.visualDisturbance)
      symptoms.push("Visual Disturbance");
    if (donorData.systemicInquiry?.neuro?.headache) symptoms.push("Headache");
    if (donorData.systemicInquiry?.neuro?.limbWeakness)
      symptoms.push("Limb Weakness");

    // Gynecology
    if (donorData.systemicInquiry?.gynecology?.pvBleeding)
      symptoms.push("PV Bleeding");
    if (donorData.systemicInquiry?.gynecology?.menopause)
      symptoms.push("Menopause");
    if (donorData.systemicInquiry?.gynecology?.menorrhagia)
      symptoms.push("Menorrhagia");
    if (donorData.systemicInquiry?.gynecology?.lrmp)
      symptoms.push("Last Regular Menstrual Period");
  ...
  const getFamilyHistoryList = () => {
    const history = [];
    if (donorData.familyHistory?.dm)
      history.push(`DM: ${donorData.familyHistory.dm}`);
    if (donorData.familyHistory?.htn)
      history.push(`HTN: ${donorData.familyHistory.htn}`);
    if (donorData.familyHistory?.ihd)
      history.push(`IHD: ${donorData.familyHistory.ihd}`);
    if (donorData.familyHistory?.stroke)
      history.push(`Stroke: ${donorData.familyHistory.stroke}`);
    if (donorData.familyHistory?.renal)
      history.push(`Renal: ${donorData.familyHistory.renal}`);
  ...
  const getSubstanceUseList = () => {
    ...
    if (donorData.substanceUse?.other)
      substances.push(donorData.substanceUse.other);
  ...
  const getOralExaminationList = () => {
    const oralFindings = [];
    if (donorData.examination?.oral?.dentalCaries)
      oralFindings.push("Dental Caries");
    if (donorData.examination?.oral?.oralHygiene)
      oralFindings.push("Oral Hygiene Issues");
    if (donorData.examination?.oral?.satisfactory)
      oralFindings.push("Satisfactory");
    if (donorData.examination?.oral?.unsatisfactory)
      oralFindings.push("Unsatisfactory");
  ...
  const getRespiratoryFindings = () => {
    const findings = [];
    if (donorData.examination?.respiratory?.auscultation)
      findings.push("Abnormal Auscultation");
  ...
  const getAbdominalFindings = () => {
    const findings = [];
    if (donorData.examination?.abdomen?.hepatomegaly)
      findings.push("Hepatomegaly");
    if (donorData.examination?.abdomen?.splenomegaly)
      findings.push("Splenomegaly");
    if (donorData.examination?.abdomen?.renalMasses)
      findings.push("Renal Masses");
  ...
  const getNeurologicalFindings = () => {
    const findings = [];
    if (donorData.examination?.neurologicalExam?.cranialNerves)
      findings.push("Cranial Nerves Abnormal");
    if (donorData.examination?.neurologicalExam?.upperLimb)
      findings.push("Upper Limb Abnormal");
    if (donorData.examination?.neurologicalExam?.lowerLimb)
      findings.push("Lower Limb Abnormal");
    if (donorData.examination?.neurologicalExam?.coordination)
      findings.push("Coordination Abnormal");
    return findings.length > 0 ? findings.join(", ") : "Normal";
  };
  ...
  const getStatusBadge = () => {
    ...
    if (status === "evaluating") {
      return (
        <Badge className="bg-yellow-100 text-yellow-800">Evaluating</Badge>
      );
    }
    if (status === "rejected") {
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {donorData.dateOfBirth
                    ? formatDateToDDMMYYYY(donorData.dateOfBirth)
                    : "N/A"}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {getValue(donorData.contactDetails)}
                </p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-600">
                ...
                <p className="text-gray-900">
                  {getValue(donorData.emailAddress)}
                </p>
              </div>
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-gray-900">
                  {getValue(donorData.relationToRecipient)}
                </p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-600">
                ...
                <p className="text-gray-900">
                  {getValue(donorData.relationType)}
                </p>
              </div>
              <div className="md:col-span-2">
                <label className="text-sm font-medium text-gray-600">
                ...
                {donorData.status === "assigned" ? (
                  <p className="text-gray-900">
                    Assigned to:{" "}
                    {donorData.assignedRecipientName ||
                      donorData.assignedRecipientPhn ||
                      "N/A"}
                  </p>
                ) : (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="text-gray-900">
                  {getValue(donorData.drugHistory)}
                </p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-600">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900">
                      {getValue(donorData.examination?.height)}
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">
                    ...
                    <p className="text-gray-900">
                      {getValue(donorData.examination?.weight)}
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">
                    ...
                    <p className="text-gray-900">
                      {getValue(donorData.examination?.bmi)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Badge
                      variant={
                        donorData.examination?.clubbing
                          ? "destructive"
                          : "secondary"
                      }
                    >
                  ...
                  <div>
                    ...
                    <Badge
                      variant={
                        donorData.examination?.ankleOedema
                          ? "destructive"
                          : "secondary"
                      }
                    >
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900">
                      {getValue(donorData.examination?.cvs?.bp)}
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">
                    ...
                    <p className="text-gray-900">
                      {getValue(donorData.examination?.cvs?.pr)}
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">
                    ...
                    <Badge
                      variant={
                        donorData.examination?.cvs?.murmurs
                          ? "destructive"
                          : "secondary"
                      }
                    >
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900">
                      {getValue(donorData.examination?.respiratory?.rr)}
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">
                    ...
                    <p className="text-gray-900">
                      {getValue(donorData.examination?.respiratory?.spo2)}
                    </p>
                  </div>
                  <div className="md:col-span-2">
                    <label className="text-sm font-medium text-gray-600">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900">
                    {donorData.immunologicalDetails?.bloodGroup?.d &&
                    donorData.immunologicalDetails?.bloodGroup?.r
                ...
                <div>
                  ...
                  <p className="text-gray-900">
                    {getValue(
                      donorData.immunologicalDetails?.crossMatch?.tCell
                    )}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-gray-900">
                    {getValue(
                      donorData.immunologicalDetails?.crossMatch?.bCell
                    )}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-gray-900">
                    {getValue(
                      donorData.immunologicalDetails?.immunologicalRisk
                    )}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900">
                    {getValue(donorData.immunologicalDetails?.pra?.pre)}
                  </p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">
                  ...
                  <p className="text-gray-900">
                    {getValue(donorData.immunologicalDetails?.pra?.post)}
                  </p>
                </div>
                <div className="md:col-span-2">
                  <label className="text-sm font-medium text-gray-600">
                  ...
                  <p className="text-gray-900">
                    {getValue(donorData.immunologicalDetails?.dsa)}
                  </p>
                </div>
              </div>
            </CardContent>
