...
const HDMonthlyReview: React.FC<HDMonthlyReviewProps> = ({ onBack }) => {
  ...
  const loadAssessments = async () => {
    ...
    try {
      ...
      toast({
        title: "Load failed",
        description: "Could not load monthly reviews",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!date) {
      toast({
        title: "Date required",
        description: "Please select a date for this note",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        ...
        toast({
          title: "Updated",
          description: "Monthly review updated",
          variant: "default",
        });
      } else {
        ...
        toast({
          title: "Saved",
          description: "Monthly review saved",
          variant: "default",
        });
      }
  ...
  const handleEdit = async (id: number) => {
    try {
      ...
      const item = (saved || []).find(
        (s: any) => String(s.id) === String(id) || s.id === id
      );
      if (item) {
        setEditingId(id);
        setDate(
          item.reviewDate || item.date || new Date().toISOString().split("T")[0]
        );
        ...
        if (it) {
          setEditingId(id);
          setDate(
            it.reviewDate || it.date || new Date().toISOString().split("T")[0]
          );
          ...
          toast({
            title: "Not found",
            description: "Could not locate review for editing",
            variant: "destructive",
          });
        }
      }
      ...
      toast({
        title: "Load failed",
        description: "Could not load note for editing",
        variant: "destructive",
      });
    }
  };
  ...
  const filtered = saved.filter((s: any) => {
    const matchesDate = filterDate
      ? (s.reviewDate || "").startsWith(filterDate)
      : true;
    const matchesTerm = searchTerm
      ? (s.doctorNotes || "").toLowerCase().includes(searchTerm.toLowerCase())
      : true;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage clinical notes and observations for hemodialysis patients
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    setNotes("");
                    setDate(formatDateToInputValue(new Date().toISOString()));
                  }}
                  className="flex-1"
                >
                  Clear
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            {!showSaved ? (
              <div className="text-sm text-muted-foreground">
                Saved notes are hidden. Click <strong>Show</strong> to view
                saved follow-up notes for this patient.
              </div>
            ) : loading ? (
              <p>Loading...</p>
            ) : filtered.length === 0 ? (
                      ...
                      <div className="flex flex-col gap-2 text-right">
                        ...
                        <div className="flex flex-col gap-2">
                          <Button
                            ...
                            onClick={() =>
                              navigate(
                                `/investigation/monthly-review/${fu.id}?patient=${phn}`
                              )
                            }
                          >
