...
const RecipientSummaryOverlay: React.FC<RecipientSummaryOverlayProps> = ({
  form,
  onClose,
}) => {
  ...
  const copyAsReport = () => {
    ...
    ["name", "phn", "age", "gender"].forEach((k) =>
      lines.push(`${k}: ${(form as any)[k] ?? "—"}`)
    );
    lines.push("");
    ...
    [
      ...
      "relationToRecipient",
      "relationType",
    ].forEach((k) => lines.push(`${k}: ${(form as any)[k] ?? "—"}`));
    lines.push("");
    lines.push("Comorbidities:");
    Object.entries(comorbidities).forEach(([k, v]) =>
      lines.push(
    ...
    Object.entries(rrt).forEach(([k, v]) =>
      lines.push(
        `${k}: ${typeof v === "boolean" ? (v ? "Yes" : "No") : v || "—"}`
      )
    );
    if (transfusions.length) {
      ...
      transfusions.forEach((t: any) =>
        lines.push(
          `${t.date || "—"} | ${t.indication || "—"} | ${t.volume || "—"}`
        )
      );
    }
    lines.push("");
    ...
    if (immuno.bloodGroup) {
      Object.entries(immuno.bloodGroup).forEach(
        ([k, v]) => (immunoFlat[`bloodGroup.${k}`] = v)
      );
    }
    if (immuno.crossMatch) {
      Object.entries(immuno.crossMatch).forEach(
        ([k, v]) => (immunoFlat[`crossMatch.${k}`] = v)
      );
    }
    ...
    if (immuno.immunologicalRisk)
      immunoFlat["immunologicalRisk"] = immuno.immunologicalRisk;
    Object.entries(immunoFlat).forEach(([k, v]) =>
      lines.push(`${k}: ${v || "—"}`)
    );
  ...
  const exportPrint = () => {
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Recipient Assessment</title><style>body{font-family:Arial,Helvetica,sans-serif;padding:24px;color:#111} h1{font-size:20px;margin:0 0 12px} h2{font-size:16px;margin:24px 0 8px} table{border-collapse:collapse;width:100%;margin-bottom:12px} th,td{border:1px solid #ddd;padding:6px;font-size:12px;text-align:left} .section{margin-bottom:12px} .kv{display:grid;grid-template-columns:160px 1fr;gap:4px 12px;font-size:12px} .kv div.label{font-weight:600} .badge{display:inline-block;padding:2px 6px;border-radius:4px;background:#eef} </style></head><body><h1>Recipient Assessment Summary</h1><h2>Basic Information</h2><div class="kv">${["name", "phn", "age", "gender"].map((k) => `<div class='label'>${k}</div><div>${(form as any)[k] ?? "—"}</div>`).join("")}</div><h2>Donor Details</h2><div class="kv">${["donorName", "donorPhn", "donorBloodGroup", "relationToRecipient", "relationType"].map((k) => `<div class='label'>${k}</div><div>${(form as any)[k] ?? "—"}</div>`).join("")}</div><h2>Comorbidities</h2><div class="kv">${Object.entries(
      ...
      .join(
        ""
      )}</div>${transfusions.length ? `<h2>Transfusion History</h2><table><thead><tr><th>Date</th><th>Indication</th><th>Volume</th></tr></thead><tbody>${transfusions.map((t: any) => `<tr><td>${t.date || "—"}</td><td>${t.indication || "—"}</td><td>${t.volume || "—"}</td></tr>`).join("")}</tbody></table>` : ""}<h2>Immunological Details</h2><div class="kv">${(() => {
      const arr: string[] = [];
      if (immuno.bloodGroup) {
      ...
      ["praPre", "praPost", "dsa", "immunologicalRisk"].forEach((k) => {
        if ((immuno as any)[k])
          arr.push(
            `<div class='label'>${k}</div><div>${(immuno as any)[k] || "—"}</div>`
          );
      });
      return arr.join("");
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-2xl md:max-w-4xl lg:max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            {form.phn ? `${form.name || ""} — ${form.phn}` : form.name || ""}
          </DialogDescription>
        </DialogHeader>

        <div className="flex items-center justify-end gap-2 mb-2">
          <Button size="sm" variant="outline" onClick={copyAsReport}>
            Copy as report
          </Button>
          <Button size="sm" variant="outline" onClick={exportPrint}>
            Export / Print
          </Button>
              ...
              <AccordionContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Field
                    ...
                    value={(form as any).donorBloodGroup}
                  />
            ...
            <AccordionItem value="immuno">
              ...
              <AccordionContent>
                <div className="space-y-4">
                  {immuno.bloodGroup && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {Object.entries(immuno.bloodGroup).map(([k, v]) => (
                        <Field key={k} label={`Blood Group ${k}`} value={v} />
                      ))}
                    </div>
                  )}
                  {immuno.crossMatch && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {Object.entries(immuno.crossMatch).map(([k, v]) => (
                        <Field key={k} label={`Cross Match ${k}`} value={v} />
                      ))}
                    </div>
                  )}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Field
                      ...
                      value={immuno.immunologicalRisk}
                    />
                  </div>
                </div>
              </AccordionContent>
                ...
                {transfusions.length ? (
                  <div className="space-y-2">
                    {transfusions.map((t: any, i: number) => (
                      <div
                        key={i}
                        className="text-sm p-2 rounded border bg-blue-50 dark:bg-slate-800"
                      >
          ...
          <div>
            ...
            <div className="p-3 bg-muted rounded-md">
              <p className="text-sm text-black dark:text-white whitespace-pre-wrap">
                {(form as any).completedBy?.staffName ||
                  (form as any).filledBy ||
                  "—"}
              </p>
            </div>
          </div>
