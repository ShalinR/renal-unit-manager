...
const HDWeekScheduleFloating: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const loadWeek = async () => {
    setLoading(true);
    try {
      const promises = days.map((d) => hdScheduleApi.getAppointmentsByDate(fmt(d)).catch(() => []));
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('hd-schedule-updated', handler as EventListener);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [center]);

  const isTaken = (dateStr: string, slotId: string) => (appointmentsByDate[dateStr] || []).some((a) => a.slotId === slotId);

  const getAppointment = (dateStr: string, slotId: string) => (appointmentsByDate[dateStr] || []).find((a) => a.slotId === slotId);
  ...
  const handleBook = async (dateStr: string, slotId: string) => {
    ...
    try {
      await hdScheduleApi.bookAppointment({ phn: patient.phn, patientName: patient.name || patient.phn, date: dateStr, slotId });
      toast({ title: 'Booked', description: `Booked ${dateStr} ${slotId}` });
    } catch (err: any) {
      toast({ title: 'Booking failed', description: err?.message || 'Unable to book', variant: 'destructive' });
    } finally {
      setBookingId(null);
    }
  };
  ...
  const handleCancel = async (id: number) => {
    ...
    try {
      ...
      toast({ title: 'Cancel failed', description: 'Unable to cancel', variant: 'destructive' });
    } finally {
      setBookingId(null);
    }
  };
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-5 flex-shrink-0">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => setCenter(new Date(center.getFullYear(), center.getMonth(), center.getDate() - 7))}
            ...
            <>
              <div className="grid grid-cols-7 gap-4 mb-6">
                {days.map((d) => {
                  ...
                  return (
                    <div key={dateStr} className={`flex flex-col rounded-xl overflow-hidden border-2 transition ${isToday ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white'}`}>
                      {/* Day Header */}
                      <div className={`p-3 ${isToday ? 'bg-blue-100' : 'bg-gradient-to-b from-gray-50 to-gray-25'} border-b border-gray-200`}>
                        <div className="font-bold text-gray-900">{dayName}</div>
                        <div className="text-lg font-bold text-blue-600">{dayNum}</div>
                      ...
                      <div className="space-y-2 p-3 flex-1">
                        {DEFAULT_SLOTS.map((s) => {
                          ...
                          const isLoading = bookingId === `${dateStr}-${s.id}` || bookingId === `cancel-${appt?.id}`;
                          return (
              ...
              {patient?.phn && (
                <div className="bg-gradient-to-r from-green-50 to-blue-50 border-l-4 border-green-500 rounded-lg p-4 flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-gray-900">Selected Patient</div>
                    <div className="text-sm text-gray-600">{patient.name || patient.phn}</div>
                  </div>
                </div>
              )}
              {!patient?.phn && (
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-l-4 border-amber-500 rounded-lg p-4 flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-gray-900">Select a Patient</div>
                    <div className="text-sm text-gray-600">Please select a patient from the search to book appointments.</div>
                  </div>
                </div>
              )}
            </>
