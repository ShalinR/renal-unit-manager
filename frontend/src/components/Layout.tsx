...
const Layout = ({ children }: LayoutProps) => {
  ...
  // 🔍 SEARCH BY PHN → /ward-management?phn=...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    const raw = (searchQuery || "").trim();
    if (!raw) return;

    // Normalize so "443221", "phn-443221" → "PHN-443221"
    const norm = (() => {
      const t = raw.toUpperCase();
      if (t.startsWith("PHN-")) return t;
      if (/^\d+$/.test(t)) return `PHN-${t}`;
      return t;
    })();

    navigate(`/ward-management?phn=${encodeURIComponent(norm)}`);
  };
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <div className="relative w-80">
                    ...
                    <Input
                      type="text"
                      placeholder="Search by PHN (e.g. PHN-443221)"
                      value={searchQuery}
